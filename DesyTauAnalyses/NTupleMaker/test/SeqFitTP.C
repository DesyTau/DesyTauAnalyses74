...
void FitTP(TString SampleName,
  ...
  fitFuncPass->SetParLimits(2,0.5*Width1,3*Width1);
  fitFuncPass->SetParameter(3,Width2);
  fitFuncPass->SetParLimits(3,0.5*Width2,3*Width2);
  fitFuncPass->SetParameter(4,Width1);
  fitFuncPass->SetParLimits(4,0.5*Width1,3*Width1);
  fitFuncPass->SetParameter(5,Width2);
  fitFuncPass->SetParLimits(5,0.5*Width2,3*Width2);
  ...
  if (histPass->GetSum()<10)
    fitPass = false;
  histPass->GetXaxis()->SetTitle("m_{#mu#mu} [GeV]");
  ...
  if (fitPass) histPass->Fit("fitFuncPass","R");
  ...
  fitFuncFail->SetParLimits(2,0.8*fitFuncPass->GetParameter(2),1.1*fitFuncPass->GetParameter(2));
  fitFuncFail->SetParameter(3,fitFuncPass->GetParameter(3));
  fitFuncFail->SetParLimits(3,0.8*fitFuncPass->GetParameter(3),1.1*fitFuncPass->GetParameter(3));
  fitFuncFail->SetParameter(4,fitFuncPass->GetParameter(4));
  fitFuncFail->SetParLimits(4,0.8*fitFuncPass->GetParameter(4),1.1*fitFuncPass->GetParameter(4));
  fitFuncFail->SetParameter(5,fitFuncPass->GetParameter(5));
  fitFuncFail->SetParLimits(5,0.8*fitFuncPass->GetParameter(5),1.1*fitFuncPass->GetParameter(5));
  ...
  if (histFail->GetSum()<10)
    fitFail = false;
  histFail->GetXaxis()->SetTitle("m_{#mu#mu} [GeV]");
  ...
  if (fitFail) histFail->Fit("fitFuncFail","R");
...
void SeqFitTP(TString fileName = "SingleMu_Run2015B_New",
  ...
  TString PtBins[6]  = {"Pt10to15","Pt15to20","Pt20to30",
			"Pt30to40","Pt40to60","PtGt60"};

  if (histBaseName.Contains("ZMassMu")) {
    nPtBins = 6;
    ptBins[1] = 15;
    ptBins[2] = 19;
    ptBins[3] = 24;
