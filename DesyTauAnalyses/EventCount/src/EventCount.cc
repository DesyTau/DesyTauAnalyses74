...
class EventCount : public edm::EDAnalyzer {
  ...
  TH1D * _eventCount;
  //  TH1D * _eventCountJetBins;
  TH1D * _eventWeights;
  bool _doMC;
  //  edm::InputTag _genParticleSrc;
  edm::InputTag _hepMcSrc;
...

{
  ...
  _hepMcSrc        = iConfig.getParameter<edm::InputTag>  (  "HepMcSource"  ); 

  edm::Service<TFileService> fs;

  _eventCount = fs->make<TH1D>("EventCount","number of events",1,-0.5,0.5);
  if (_doMC) {
    //    _eventCountJetBins = fs->make<TH1D>("EventCountJetBins","number of events in jet bins",11,-0.5,10.5);
    _eventWeights =  fs->make<TH1D>("EventWeights","sum of generator weights",1,-0.5,0.5);
  }
...
EventCount::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   if (_doMC) {
     ...
     //       }
     //       if (pdg == 23 || pdg == 24 || pdg ==-24 || pdg == 25 || pdg == 35 || pdg == 36) 
     //	 count_jets = true;  // Start counting partons after we find the Z
     //
     //     }
     // end of counting jets
     //     _eventCountJetBins->Fill(double(Npartons));
     edm::Handle<GenEventInfoProduct> HEPMC;
     iEvent.getByLabel(_hepMcSrc, HEPMC);
     double genweight = HEPMC->weight();
     _eventWeights->Fill(0.0,genweight);
   }
